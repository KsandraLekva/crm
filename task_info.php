...
if (isset($_SESSION['auth']))
{
    ...
    $task_id = $_SESSION['view_task'];
	$project_id = $_SESSION['project_id'];

	//Если форма отправлена...
	if ( !empty($_REQUEST['task_name']) and !empty($_REQUEST['task_desc']) and !empty($_REQUEST['date']) ) {
        ...
        $query = "UPDATE `crm_tasks` SET `task_name`= '$task_name', `desc`= '$desc', `deadline`= '$date' WHERE `id`='$task_id'";
...
/*
		...
		$query = "SELECT * FROM `crm_tasks` WHERE `id` = '$task_id'"; 
	$res_query = mysqli_query($connection, $query);
		...
		<div class="section">
			...
			<div class="container">
				...
				<div class="row">
					...
					<!-- section title -->
					<div class="col-md-12">
						<div class="section-title"><? foreach ($task_info as $key => $value): ?>
							<h3 class="title">Информация о задаче <?=$value['task_name']?></h3>
						</div>
					</div>
					<!-- /section title -->

                    <form id="login-form" action='task_info.php' method='POST'>
                    <input type="text" value="<?=$value['task_name']?>" name="task_name" class="input" style="width:500px;" placeholder="Название задачи"><br><br>
					<textarea type="text" name="task_desc" placeholder="Описание задачи" style="width:700px; height: 150px;" cols="2" class="input" rows="3"><?=$value['desc']?></textarea><br><br>

                    <?  $createDate = new DateTime($value['deadline']);
                        $date = $createDate->format('Y/m/d'); ?>
					<input class="input" value="<?=$date?>" style="width:220px;" placeholder="Срок сдачи yyyy/mm/dd"
					onkeyup="
						var v = this.value;
						if (v.match(/^\d{4}$/) !== null) {
							this.value = v + '/';
					...
					maxlength="10" name="date" type="text"  size="30"><br><br><? endforeach; ?>

                    <? if ($role_id==1) echo '<input type="submit" class="primary-btn order-submit" value="Обновить задачу">'?>
